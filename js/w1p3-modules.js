async function fetchPhotos (el, endpoint) {
	if (!endpoint || !el || typeof(el) === 'undefined') return;
	try {
		let response = await fetch(endpoint);
		if (!response.ok) throw response;
		let photos = await response.json();
		el.innerHTML = getPhotosHTML(photos);

	} catch (error) {
		console.warn(error);
		el.innerHTML = '<p>Error retrieving photos.</p>';
	}
}
